...
const loadCommands = () => {
  ...
  for (const file of files) {
    try {
      ...
      log('error', `خطأ في تحميل الأمر ${file}: ${error.message}`);
    }
  }
  return commands;
};
...
const handleCommand = async ({ message, args, event, api, Users, Threads, commands }) => {
  try {
    ...
    if (global.client.config.adminOnlyMode && !hasPermission(event.senderID, { adminOnly: true })) {
      return api.sendMessage('⚠️ البوت حالياً في وضع المشرفين فقط. يمكن للمشرفين فقط استخدام الأوامر.', event.threadID);
    }
    ...
    if (global.client.config.features.cooldown && !checkCooldown(event.senderID, command.config.name, command.config.countDown)) {
      return api.sendMessage(`⏱️ يرجى الانتظار ${command.config.countDown} ثانية قبل استخدام هذا الأمر مرة أخرى.`, event.threadID);
    }
